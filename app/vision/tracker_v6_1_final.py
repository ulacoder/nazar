    ...
    def update(self,detections,timestamp,frame_index=None,frame_shape=None):
        ...
        for col,det in enumerate(detections):
            ...
            if not first_public_person:
                ...
                # V6.1 COUNT-PRIORITY
                #
                # A genuinely new person should normally increase the
                # physical number of detections. Count is now a strong
                # prior rather than a weak hint.
                ...
                # someone left and a genuinely different person appeared.
                #
                # Require stronger and longer appearance evidence. This is
                # deliberately harder than an ordinary entrance.
                confirmed_replacement=(
                    not count_increased
                    and age >= 1.50
                    and hits >= 8
                    and closest_known is not None
                    and closest_known >= .18
                )

                # V6 used an unconditional timeout here. That created
                # public IDs merely because an uncertain detection existed
                # long enough. In V6.1 timeout alone is NOT identity
                # evidence.
                hard_timeout=False
