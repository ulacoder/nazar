...
class PoseAdapter:
    ...
    def infer(self, frame):
        ...
        result=self.model.track(frame,persist=True,tracker="bytetrack.yaml",conf=self.conf,imgsz=640,device=0 if self.device.type=="cuda" else "cpu",verbose=False)[0]
        self.calls+=1;self.latency_ms=(time.perf_counter()-start)*1000
        if result.boxes is None or result.keypoints is None: return []
        boxes=result.boxes.xyxy.detach().cpu().numpy()
        scores=result.boxes.conf.detach().cpu().numpy()
        points=result.keypoints.data.detach().cpu().numpy()
        ids=result.boxes.id.detach().cpu().numpy().astype(int).tolist() if result.boxes.id is not None else [None]*len(boxes)
        return [Detection(tuple(map(float,box)),np.asarray(kp),float(score),tid) for box,kp,score,tid in zip(boxes,points,scores,ids)]
